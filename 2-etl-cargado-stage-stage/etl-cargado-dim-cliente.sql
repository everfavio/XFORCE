...
-- creando la funcion
create or replace function two_etl_stage_dim_cliente()
returns boolean
as $$
  --para errores
  ...
  BEGIN
    ...
    nombre_proceso = 'two_etl_stage_dim_cliente';
    cantidad_registros = 0;
    -- limpieza de customers
    truncate table  stage_star_dim_cliente;
    insert into stage_star_dim_cliente (
      idw_cliente     ,
      id_cliente      ,
      cliente         ,
      nombre_contacto ,
    ...
    select
      coalesce(
       (select idw_cliente from star.dim_cliente
         where id_cliente = stp.customerid), -1 ) idw_cliente,
      stp.customerid        ,
      stp.companyname       ,
      stp.contacttitle      ,
      stp.city              ,
